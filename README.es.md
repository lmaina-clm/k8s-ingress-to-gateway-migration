...
├── docs/                              ← en inglés (default); versiones `.es.md` para español
│   ├── 01-prerequisites.es.md         ← qué necesitas antes de empezar
│   ├── 02-architecture.es.md          ← decisiones de diseño y por qué
│   ├── 03-ingress-vs-gateway.es.md    ← comparación conceptual y mapeo 1:1
│   ├── 04-migration-runbook.es.md     ← el runbook paso a paso ← EMPIEZA AQUÍ
│   ├── 05-zero-downtime.es.md         ← análisis y estrategia
│   ├── 06-rollback.es.md              ← plan de reversión
│   ├── 07-troubleshooting.es.md       ← problemas comunes
│   ├── 08-faq.es.md
│   └── 09-fast-validation-runbook.es.md  ← validación rápida en EKS efímero (~45 min)
├── manifests/
...
1. **`docs/01-prerequisites.es.md`** — confirma que tu entorno cumple los requisitos.
2. **`docs/03-ingress-vs-gateway.es.md`** — entiende qué cambia conceptualmente.
3. **`docs/04-migration-runbook.es.md`** — el runbook ejecutable, con checkpoints y criterios de éxito en cada fase.
4. **`docs/05-zero-downtime.es.md`** — entiende los riesgos reales y cómo mitigarlos antes de tocar producción.
...
Si alguno de estos no se cumple, **`docs/05-zero-downtime.es.md`** documenta los workarounds y el costo de cada uno. La estrategia base es la siguiente:
...
Las versiones anteriores de NGF (1.x) usan un esquema de instalación distinto. Si estás en 1.x, ver `docs/07-troubleshooting.es.md` para el upgrade path.
