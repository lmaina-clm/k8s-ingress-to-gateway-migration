...
- Si tu DNS no soporta weighted routing, hay un fallback con dos hostnames distintos documentado en `05-zero-downtime.es.md`.
...
Habrá que replicarlos para NGINX Gateway Fabric **antes** del cutover — los nombres de métricas cambian. Ver `07-troubleshooting.es.md` sección "Observabilidad".

## 6. Aplicación

Validaciones específicas de la aplicación que vas a migrar (no la demo, sino la real):

- [ ] ¿Usa anotaciones de `ingress-nginx`? Lista cuáles. Algunas tienen equivalente directo en Gateway API, otras requieren `NginxProxy` o policies custom de NGF. Ver `03-ingress-vs-gateway.es.md` tabla de mapeo.
...
✅ Si todo está marcado → continúa a [`02-architecture.es.md`](./02-architecture.es.md).
