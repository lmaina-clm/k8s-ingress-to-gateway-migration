...
Ver `02-architecture.es.md`. Resumen:
...
Sí, sin configuración especial. NGF detecta el upgrade HTTP→WS automáticamente. **Pero los WebSockets son el caso más complicado para zero-downtime** — ver `05-zero-downtime.es.md`.
...
Sí, los específicos de `ingress-nginx`. Los nombres de métricas cambian. Tienes que recrearlos contra las métricas de NGF antes del cutover. Ver `07-troubleshooting.es.md` sección "Observabilidad".
